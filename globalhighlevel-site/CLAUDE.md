...
- **Render-time backstop (v0.2.12.0):** `build.py` (`nofollow_affiliate_links`) stamps `rel="nofollow sponsored"` on EVERY anchor whose href carries `fp_ref=` — appended to any existing rel, stored JSON untouched. Firehose-era bodies carried followed paid links (Google paid-link policy risk); sitewide followed-affiliate-anchor count is now 0. Author-written rel attributes are still expected, but the pass guarantees the invariant.
