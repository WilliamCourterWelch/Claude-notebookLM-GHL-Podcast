...
0. **Run the pre-deploy test suite** — `python3 -m pytest scripts/ -q` from `globalhighlevel-site/` (all must pass, incl. the editorial-debt gate), then `python3 build.py` + `python3 verify.py`. Full suite doc: `globalhighlevel-site/CLAUDE.md` § "Tests — pre-deploy gate suite" (added v0.3.4.0).
