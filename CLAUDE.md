...
## Site Structure
- 4 languages: English (default), Español (`/es/`), India (`/in/`), Arabic (`/ar/`, RTL) — Arabic was removed with the retired pipeline, then re-added as a static-site section in v0.3.0.0 (2026-07-27): `/ar/` hub, RTL rendering, native-Arabic trial landing at `/ar/trial/`
...
- **`/trial/` is an ATTRIBUTION URL, not an SEO landing.** Full content page (~1,900 words) that pitches both GHL + Extendly affiliates, GA4-tracked CTAs, intentionally `Disallow`'d in `robots.txt` so it doesn't cannibalize organic SERPs. A parallel SEO-indexable blog post exists (`/blog/gohighlevel-free-trial-30-days-extended/`). Do NOT unblock, thin out, or migrate its content. **`/start/` and `/coupon/` are RETIRED** (no longer built; `_redirects` 301s them to the money page) and were deliberately UNBLOCKED in robots.txt on 2026-07-22 so crawlers can see the 301s and **external** backlink equity flows (the internal `/start/`-path CTAs were repointed in v0.2.11.0 — they now go straight to the money page `/blog/gohighlevel-free-trial-30-days-extended/`, still `rel=nofollow`; direct-affiliate CTAs are unchanged; TODOS T5 shipped in its D2-superseded form) — do NOT re-add their `Disallow` lines, and do NOT point new in-post CTAs at `/start/` or `/coupon/`. Full rules in `globalhighlevel-site/CLAUDE.md`.
...
0. **Run the pre-deploy test suite** — `python3 -m pytest scripts/ -q` from `globalhighlevel-site/` (all must pass, incl. the content gates: editorial-debt, trial-claim residual, and the retired-timer-premise gate added v0.3.9.0), then `python3 build.py` + `python3 verify.py`. Full suite doc: `globalhighlevel-site/CLAUDE.md` § "Tests — pre-deploy gate suite" (added v0.3.4.0). **Order matters: `verify.py` reads BUILT output, so run `build.py` first** — verifying against a stale build reports violations that do not exist (cost a real session's debugging on 2026-07-31).
