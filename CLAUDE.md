...
- **`/trial/` is an ATTRIBUTION URL, not an SEO landing.** Full content page (~1,900 words) that pitches both GHL + Extendly affiliates, GA4-tracked CTAs, intentionally `Disallow`'d in `robots.txt` so it doesn't cannibalize organic SERPs. A parallel SEO-indexable blog post exists (`/blog/gohighlevel-free-trial-30-days-extended/`). Do NOT unblock, thin out, or migrate its content. **`/start/` and `/coupon/` are RETIRED** (no longer built; `_redirects` 301s them to the money page) and were deliberately UNBLOCKED in robots.txt on 2026-07-22 so crawlers can see the 301s and link equity flows — do NOT re-add their `Disallow` lines. Full rules in `globalhighlevel-site/CLAUDE.md`.
